...
def get_combined_img(bg_img, object_img, object_mask, bg_ul):
    ...
    #Get number of rows and columns in object image
    (nr, nc) = object_img.shape[:2]

    #Loop through color channels of object
    for b in np.arange(object_img.shape[2]):
        #Make combined image patch with dimensions from object shape
      combined_patch = combined_img[bg_ul[0]:bg_ul[0]+nr, bg_ul[1]:bg_ul[1]+nc, b]
        #If there is a zero in the object mask, use background pixels
        #if there is a one in the object mask, use object pixels
      combined_patch = combined_patch*(1-object_mask) + object_img[:,:,b]*object_mask
        #Place this patch in the proper location
